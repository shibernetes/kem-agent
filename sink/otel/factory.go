...
// factory builds the sinks exporting to an OTLP endpoint.
type factory struct{}

// NewFactory returns the factory of the OTel sink.
...
func (factory) Create(opts sink.Options, cfg sink.Config) (sink.Sink, error) {
	c, ok := cfg.(*Config)
	if !ok {
		return nil, fmt.Errorf("sink/otel: got config of type %T, want %T", cfg, (*Config)(nil))
	}
